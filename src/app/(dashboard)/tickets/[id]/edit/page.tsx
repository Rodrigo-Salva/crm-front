...
export default function EditTicketPage() {
  ...
  const [form, setForm] = useState({ subject: '', description: '', status: 'open' as string, priority: 'medium' as string });

  const load = useCallback(async () => {
    try {
      const res = await api.get<Ticket>(`/tickets/${id}`);
      setTicket(res);
      setForm({ subject: res.subject, description: res.description || '', status: res.status, priority: res.priority });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    try { await api.patch(`/tickets/${id}`, form); router.push(`/tickets/${id}`); } catch {} finally { setSaving(false); }
  };

  if (loading) return <Loading />;
  if (!ticket) return <p className="text-center py-20 text-gray-500">Ticket no encontrado</p>;
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-[var(--text)]">Editar Ticket</h1>
      </div>
      <Card>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Asunto" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} required />
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} className="block w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-[var(--primary)]" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="block w-full rounded-lg border border-[var(--border)] px-3 py-2.5 text-sm shadow-sm focus:ring-2 focus:ring-[var(--primary)]">
